...
enum SelfTests {
    static func run() {
        ...
        check(!WorkTimeLedger.shouldCount(selected: true, focused: true, running: true, suspended: true, idle: 0), "Sleeping/inactive sessions pause tracking")
        check(!WorkTimeLedger.shouldCount(selected: true, focused: true, running: false, suspended: false, idle: 0), "Exited apps stop accumulating time")
        var workLedger = WorkTimeLedger()
        workLedger.credit(project: "/first", elapsed: 5, wasActive: true, isActive: true)
        check(workLedger.seconds["/first"] == 5, "Active foreground time accrues to the selected project")
        workLedger.credit(project: "/first", elapsed: 5, wasActive: true, isActive: false)
        workLedger.credit(project: "/first", elapsed: 5, wasActive: false, isActive: true)
        workLedger.credit(project: "/first", elapsed: 3600, wasActive: true, isActive: true)
        workLedger.credit(project: "/first", elapsed: -1, wasActive: true, isActive: true)
        workLedger.credit(project: nil, elapsed: 5, wasActive: true, isActive: true)
        check(workLedger.seconds["/first"] == 5, "Idle/background transitions, sleep gaps, backwards clocks and unassigned time are excluded")
        workLedger.credit(project: "/second", elapsed: 5, wasActive: true, isActive: true)
        check(workLedger.seconds["/second"] == 5 && workLedger.seconds.values.reduce(0, +) == 10, "Project totals stay separate without double counting")
        let savedTime = try! JSONEncoder().encode(workLedger)
        let restoredTime = try! JSONDecoder().decode(WorkTimeLedger.self, from: savedTime)
        check(restoredTime.seconds == workLedger.seconds, "Work totals survive serialization and restore")
